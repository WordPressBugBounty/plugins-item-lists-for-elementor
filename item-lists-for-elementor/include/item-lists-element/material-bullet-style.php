<?php
/**
 * Material Bullets render template.
 *
...
<div class="ile-material-bullet-style" style="direction:<?php echo esc_attr( $item_lists_direction ); ?>;">
	...
	<?php
	foreach ( $settings['material_item_lists'] as $item_lists_items => $item_lists_item ) {
		...
		$item_lists_content_bg_color_hex = sanitize_hex_color( $item_lists_content_bg_color );
		$item_lists_content_bg_color_hex = $item_lists_content_bg_color_hex ? $item_lists_content_bg_color_hex : '#000000';

		$item_lists_title_color_hex = sanitize_hex_color( $item_lists_title_color );
		$item_lists_title_color_hex = $item_lists_title_color_hex ? $item_lists_title_color_hex : '#000000';

		$item_lists_content_color_hex = sanitize_hex_color( $item_lists_content_color );
		$item_lists_content_color_hex = $item_lists_content_color_hex ? $item_lists_content_color_hex : '#000000';

		if ( 'icon' === $item_lists_item['material_list_items_display_icon'] ) {
			?>
				<div class="ile-icon-box" style="background-color:<?php echo esc_attr( $item_lists_icon_bg_color_hex ); ?>;">
						...
						Icons_Manager::render_icon(
							$item_lists_item['material_list_items_icon'],
							array(
								'aria-hidden' => 'true',
								'class'       => 'ile-icon',
			...
			<?php } elseif ( 'image' === $item_lists_item['material_list_items_display_icon'] ) { ?>
				<img src="<?php echo esc_url( $item_lists_item['material_list_items_image']['url'] ); ?>" class="ile-image" />
			<?php } ?>
			<div class="ile-content-box" style="background-color:<?php echo esc_attr( $item_lists_content_bg_color_hex ); ?>">
				<h2 class="ile-title" style="color:<?php echo esc_attr( $item_lists_title_color_hex ); ?>;"><?php echo esc_html( $item_lists_title ); ?></h2>
				<p class="ile-content" style="color:<?php echo esc_attr( $item_lists_content_color_hex ); ?>;"><?php echo wp_kses_post( $item_lists_content ); ?></p>
			</div>
		</div>
		<?php } ?>
	</div>
