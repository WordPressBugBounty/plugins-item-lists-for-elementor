...
if ( ! class_exists( 'item_lists_elementor' ) ) :
	 ...
	 *
	 * The class name intentionally stays lowercase/snake_case
	 * (`item_lists_elementor`) rather than being renamed to satisfy
	 * PEAR.NamingConventions.ValidClassName: the companion "Item Lists for
	 * Elementor (Pro)" plugin's own mutual-exclusivity check looks up this
	 * exact class name at runtime, so renaming it here would silently break
	 * that integration for anyone running both plugins. See the matching
	 * `class_exists( 'item_lists_elementor' )` guard above and
	 * `item_lists_plugin_activation()`, which deactivates the Pro plugin.
	 * Grandfathered in phpcs.xml.dist rather than "fixed".
	 *
	 * @access public
	 * @since  1.0.0
	 */
	class item_lists_elementor {
		...
		/**
		 * Register the plugin's WordPress action hooks.
		 */
		...
		/**
		 * Register the widget file with Elementor.
		 */
		...
		/**
		 * Load the plugin's front-end styles.
		 */
		public function item_lists_widget_script_register() {

			wp_register_style( 'item-lists-style', ILE_URL . 'assets/css/item-lists-element.css', array(), '2.0', 'all' );
			wp_enqueue_style( 'item-lists-style' );
		}

		/**
		 * Check whether Elementor is active and hook the admin notice if not.
		 */
		...
		/**
		 * Display the admin notice shown when Elementor is not installed or activated.
		 */
		...
		public function item_lists_reviews_notices() {
			$notices = get_option( 'item_lists_reviews' );
			if ( $notices ) {
